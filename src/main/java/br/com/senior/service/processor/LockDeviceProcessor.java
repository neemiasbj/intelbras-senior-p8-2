...
public final class LockDeviceProcessor extends ResponseProcessor<LockDeviceCommand> {
    public LockDeviceProcessor(ClientCSMCommunication driver, IntelbrasService intelbrasApiService, InteractionCommands interactionCommands) {
        super(driver, intelbrasApiService, interactionCommands);
    }
    
    public void process(LockDeviceCommand message) throws Exception {
        if (message == null) {
            ...
            ResponseEntity<ResponseDeviceGenericModel> responseEntity = intelbrasApiService.getUnlockDevice(block);
            if (responseEntity.getStatusCode().is2xxSuccessful()) {
                ResponseDeviceGenericModel emergency = responseEntity.getBody();
                logger.info("Lock Device" + emergency);
